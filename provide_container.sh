...
show_help(){
    ...
    echo "usage: $0 [-d] [-i] [-p] [-r] [-t] [-u] [-h]"
    echo "  -d  dockerfile name"
    echo "  -i  image name"
    echo "  -p  prometheus configuration file name"
...
provide_container(){
    ssh -l "${user}" "${remote}" "mkdir -p ~/prometheus"
    scp "${dockerfile}" "${user}@${remote}":"~/${dockerfile}"
    scp "${promconf}" "${user}@${remote}":"~/prometheus/prometheus.yml"
    ...
    ssh -l "${user}" "${remote}" "docker run -d --net=host -v ~/prometheus:/etc/prometheus --restart=unless-stopped ${image}:${tag}"
    exit 0
}
